...
def _resolve_qt_plugin_paths() -> tuple[str, str] | tuple[None, None]:
    """Resolve Qt plugin and platform plugin directories for current environment."""
    candidates = []

    # Common conda layout
    candidates.append(Path(sys.prefix) / "Library" / "plugins")
    candidates.append(Path(sys.prefix) / "Library" / "lib" / "qt6" / "plugins")

    # Common pip/PySide6 wheel layouts
    ...
    if pyside_spec and pyside_spec.submodule_search_locations:
        ...
        candidates.append(pyside_dir / "plugins")
        candidates.append(pyside_dir / "Qt" / "plugins")

    for plugin_dir in candidates:
        platform_dir = plugin_dir / "platforms"
        if plugin_dir.is_dir() and platform_dir.is_dir():
            return str(plugin_dir), str(platform_dir)
...
def setup_environment() -> None:
    ...
    """
    # Setup Qt plugin paths only if we can resolve valid directories.
    # Invalid hardcoded paths can break plugin loading on pip-based PySide6 installs.
    qt_plugins, qt_platform_plugins = _resolve_qt_plugin_paths()
    if qt_plugins and "QT_PLUGIN_PATH" not in os.environ:
        os.environ["QT_PLUGIN_PATH"] = qt_plugins
    if qt_platform_plugins and "QT_QPA_PLATFORM_PLUGIN_PATH" not in os.environ:
